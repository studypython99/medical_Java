...
public class C0620_08 {
	...
	int num = 0; //인스턴트 변수: 객체선언 후 '참조변수명.인스턴트변수명' 으로 접근
	static int n = 0; // 클래스변수: 객체선언 없이 '클래스명.클래스변수명' 으로 접근
	public static void main(String[] args) {
		Scanner scan = new Scanner(System.in);
		
		//변수선언부분
		Stu_score[] sc = new Stu_score[10]; // 학생성적 객체배열 선언
		...
		while(true)
		{
			...
			case 1:
				while(true)
				{
					...
					// 객체선언
					sc[s_count] = new Stu_score(); // 저장공간이 생겼다.!
					// 학번저장
					sc[s_count].stuNo = stuNo1;
					System.out.println("학생번호: "+stuNo1);
					//이름저장
					System.out.println("이름 입력(0.이전페이지)>>");
			...
			case 2:
				...
				for(int i=0;i<s_count;i++) 
				{
					...
					System.out.printf("%.2f\t",sc[i].total); //합계
